...
export class Bridge {
   ...
   * @param callback
   */
  pushCallbackStack(callbackName: string, callback: AndroidBridgeCallback): void {
    if (!this.androidCallbackStack[callbackName]) {
  ...
  registIOSCallHandler(bridgeName: string): this {
    ...
    this[bridgeName] = <T = any>(data: any = {}): Promise<T> =>
      new Bridge.Promise((resolve) => {
  ...
  registAndroidCallHandler(
    ...
    this[bridgeName] = <T = any>(sendData?: any): Promise<T> =>
      new Bridge.Promise((resolve, reject) => {
        let data = sendData;
        if (!window.control) {
        ...
        if (bridgeType === BridgeType.async) {
          ...
          if (callbackName && !window[callbackName]) {
            window[callbackName] = (response: any): void => {
              ...
              this.getCallbackStack(callbackName as string).forEach((cb: AndroidBridgeCallback) => {
                cb(response);
              });
